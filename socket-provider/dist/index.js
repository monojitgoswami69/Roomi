...
function setPlaybackFromTrack(room, track, positionMs = 0, isPlaying = true) {
    const playback = {
        isPlaying,
...
function setPlaybackPaused(room, track, positionMs) {
...
function getCurrentPlaybackPosition(room) {
...
async function refreshSpotifyToken(room) {
...
async function spotifyRequest(room, path, init = {}) {
    const request = async (token) => fetch(`https://api.spotify.com/v1${path}`, {
        ...init,
        headers: {
...
async function activateSpotifyDevice(room) {
    if (!room.deviceId)
...
async function playSpotifyTrack(room, track, positionMs = 0) {
...
async function pauseSpotify(room) {
    if (!room.deviceId)
        return;
...
async function seekSpotify(room, positionMs) {
    if (!room.deviceId)
        throw new Error("Player not ready");
...
async function startNextTrack(io, roomCode) {
    const room = getRoom(roomCode);
    if (!room || nextTrackLocks.get(room.roomCode))
        return;
...
async function handleSeek(io, roomCode, positionMs) {
    const room = getRoom(roomCode);
    if (!room)
...
async function playTrackByUri(io, roomCode, uri) {
    const room = getRoom(roomCode);
    if (!room)
        throw new Error("Room not found");
...
async function togglePlayback(io, roomCode) {
    const room = getRoom(roomCode);
    if (!room)
        throw new Error("Room not found");
    if (!room.currentTrack)
...
setInterval(() => {
    for (const room of rooms.values()) {
        if (!room.playback.isPlaying || !room.playback.track)
            continue;
