...
import type { QueueItem, Track, Vote } from "@/lib/types";
...
type QueueProps = {
  ...
  /** When true, voting controls are hidden (e.g. pending guest, kicked guest). */
  canVote?: boolean;
};
...
export default function Queue({
  ...
  const visible = useMemo(() => {
    const queueRows = items.map((item) => ({ ...item, isPlaying: false }));
    if (!currentTrack) return queueRows;
    const playingRow = {
      track: currentTrack,
      upvotes: 0,
      downvotes: 0,
      score: 0,
      voters: {} as Record<string, Vote>,
      isPlaying: true,
    };
    return [playingRow, ...queueRows];
  }, [items, currentTrack]);

  const castVote = (trackId: string, vote: Vote) => {
    if (!socket || !guestId) return;
    setPendingTrackId(trackId);
    socket.emit("queue:vote", { trackId, vote }, () => {
    ...
    return (
      <div className="rounded-[28px] border border-white/8 bg-white/5 px-4 py-12 text-center backdrop-blur-xl">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Start the room with something worth spinning.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1">
      {visible.map((item, index) => {
        const currentVote = item.voters?.[guestId] ?? null;
        const isPending = pendingTrackId === item.track.id;
        const voteCount = item.score;
        return (
          <div
            key={`${index}-${item.track.id}`}
            className={`flex items-center gap-3 px-2 py-3 ${
              item.isPlaying ? "rounded-2xl bg-emerald-500/[0.06]" : ""
            }`}
          >
            ...
            {item.track.albumArt ? (
              <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-2xl shadow-[0_12px_30px_rgba(2,6,23,0.35)]">
                <img
                  ...
                  className={`h-full w-full object-cover ${
                    item.isPlaying ? "brightness-[0.42]" : ""
                  }`}
                />
                {item.isPlaying ? (
              ...
              <div
                className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-white/8 bg-slate-900/70 ${
                  item.isPlaying ? "relative overflow-hidden" : ""
                }`}
              >
            ...
            <div className="min-w-0 flex-1">
              <p
                className={`truncate text-sm font-semibold tracking-tight ${
                  item.isPlaying ? "text-emerald-100" : "text-slate-100"
                }`}
              >
                {item.track.title}
              </p>
              <p
                className={`mt-1 truncate text-xs ${
                  item.isPlaying ? "text-emerald-200/70" : "text-slate-400"
                }`}
              >
                {item.track.artist}
              </p>
